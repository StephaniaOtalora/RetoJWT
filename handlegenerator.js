...
let mongo = require("./lib/utils/mongo");
...
class HandlerGenerator {
  login(req, res) {
    ...
    mongo.getDatabase((db) => {
      mongo.findUser(db, username, (user) => {
        mockedUsername = user[0].user;
        mockedPassword = CryptoJS.AES.decrypt(user[0].password,secret.secret);
